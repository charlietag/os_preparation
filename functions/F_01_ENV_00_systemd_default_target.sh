...
if [[ -z "${check_target}" ]]; then
  if [[ "${wrong_default_target_found}" = "force" ]]; then
    ...
    echo "--------------------------------------------"
    echo "It is highly recommended to: \"REINSTALL Whole CentOS using \"${default_target}\""
    echo ""
    echo "Or please be sure to set default target to \"${default_target}\" the reboot first"
    echo ""
    echo "-- HOW TO: --"
